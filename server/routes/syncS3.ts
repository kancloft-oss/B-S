...
syncS3Router.post('/', async (req, res) => {
  ...
  const S3_BUCKET_NAME = process.env.S3_BUCKET_NAME || 'brusher-s3';
  const S3_CONFIG = { endpoint: S3_ENDPOINT, region: S3_REGION, accessKey: S3_ACCESS_KEY, secretKey: S3_SECRET_KEY, bucket: S3_BUCKET_NAME };

  res.json({ message: 'Синхронизация запущена в фоновом режиме', success: true });
  
  (async (config) => {
    addLog(req, `--- АВТОМАТИЧЕСКАЯ СИНХРОНИЗАЦИЯ S3 ЗАПУЩЕНА ---`);
    addLog(req, `Endpoint: ${config.endpoint}, bucket: ${config.bucket}`);
    ...
    try {
        ...
        try {
            ...
            addLog(req, `Файл offers.xml не найден в S3 или ошибка при чтении. Цены и остатки установлены в 0.`);
        }
